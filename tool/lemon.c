...
**
** LEMON LALR(1) 解析器生成器完整源代码
**
** 本文件包含 LEMON LALR(1) 解析器生成器的所有源代码（包括头文件）。
** 源代码已合并为单一文件，以便于将 LEMON 包含在其他程序的源代码树和 Makefile 中。
**
** LEMON 解析器生成器概述：
**
** 1. 核心功能：
**    - LALR(1) 解析器生成器，类似于 YACC/BISON
**    - 专门为 SQLite SQL 解析器开发而优化
**    - 生成无冲突的解析表和高效的解析代码
**    - 支持语法错误恢复和详细的错误报告
**
** 2. 技术特点：
**    - 基于 lookahead-1 的 LALR 解析算法
**    - 支持优先级和结合性规则定义
**    - 自动处理左递归和消除歧义
**    - 生成简洁、高效的 C 代码
**
