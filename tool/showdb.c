...
** SQLite 数据库文件内容显示工具
**
** 一个用于显示 SQLite 数据库文件全部或部分内容的实用工具。
** 该工具能够解析和展示数据库文件的内部结构，包括页面内容、
** B-Tree 结构、空闲列表、溢出页面等详细信息。
**
** 工具概述：
**
** showdb 是 SQLite 提供的命令行诊断工具，专门用于检查和分析
** 数据库文件的物理结构和逻辑布局。它直接读取数据库文件，
** 不需要通过 SQLite 引擎，能够提供原始的字节级信息。
**
**
** 主要功能：
**
...
**
** 2. 页面内容显示：
**    - 以十六进制和 ASCII 格式显示页面内容
**    - 支持指定页面范围或单个页面的显示
**    - 提供字节偏移和内容映射
**
**
** 3. B-Tree 结构解析：
**    - 识别和解析 B-Tree 页面类型（表页、索引页）
**    - 分析页面头部信息和单元格结构
**    - 显示键值对、行 ID、溢出页面等详细信息
**
**
** 4. 单元格详细解码：
**    - 完全解码单元格的内容和结构
...
**    - 显示页面映射表内容
**    - 分析页面类型和父子关系
**    - 支持 autovacuum 模式的数据库
**
**
** 技术特点：
**
** 底层访问：
** - 直接读取数据库文件，绕过 SQLite VFS 层
** - 支持原始模式和 SQLite API 模式两种访问方式
** - 提供字节级别的精确信息
** - 兼容不同平台和文件系统
**
** 解析能力：
...
**
** # 显示数据库头部信息
** ./showdb database.db dbheader
**
** # 显示指定页面范围
** ./showdb database.db 100..150
** ````
**
** B-Tree 分析：
** ```bash
** # 解码 B-Tree 页面
** ./showdb database.db 100b
**
** # 解码页面并显示内容
** ./showdb database.db 100bc
**
** # 显示页面布局映射
** ./showdb database.db 100bm
**
** # 解码特定单元格
** ./showdb database.db 100bd5
** ````
**
** 系统分析：
** ```bash
...
**
** # PTRMAP 分析
** ./showdb database.db ptrmap
**
** # 空闲列表分析
** ./showdb database.db 5t
** ````
**
** 输出格式：
**
