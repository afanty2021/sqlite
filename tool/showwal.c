/*
** SQLite Write-Ahead Log (WAL) 文件内容显示工具
**
** 一个用于显示和分析 SQLite WAL 文件内容的实用工具。
** 该工具能够解析和展示 WAL 文件的内部结构，包括文件头、
** 帧内容、校验和验证等详细信息。
**
** 工具概述：
**
** showwal 是 SQLite 提供的专门用于检查 WAL 文件的诊断工具。
** WAL (Write-Ahead Logging) 是 SQLite 的一种预写日志模式，
** 允许并发读写并提高数据库性能。该工具帮助开发者理解
** WAL 文件的结构和事务日志的详细信息。
**
**
** 主要功能：
...
**
** # 显示 WAL 头部信息
** ./showwal database.wal header
**
** # 显示指定帧范围
** ./showwal database.wal 100..150
** ````
**
** 帧分析：
** ```bash
** # 显示单个帧的完整内容
** ./showwal database.wal 100
**
** # 解码帧中的 B-Tree 页面
** ./showwal database.wal 100b
**
** # 显示页面内容映射
** ./showwal database.wal 100bc
** ````
**
** 文件管理：
** ```bash
** # 截断 WAL 文件到指定帧（Unix）
** ./showwal database.wal 100truncate
** ````
**
** 输出格式：
**
