...
** SQLite 正则表达式扩展实现
**
** 本文件中的代码实现了一个紧凑而高效的正则表达式匹配器，
** 用于对 UTF8 文本进行 POSIX 扩展正则表达式匹配。
**
** 扩展功能概述：
**
** 1. 核心匹配功能：
**    - 实现了完整的 POSIX 扩展正则表达式语法
**    - 支持 UTF8 文本的 Unicode 字符匹配
**    - 提供函数和操作符两种使用方式
**    - 紧凑但高效的算法实现
**
** 2. SQLite 集成：
**    - 注册为 SQLite 扩展函数 regexp(A,B)
**    - 自动生成 B regexp A 操作符支持
**    - 完全集成到 SQLite 查询引擎
**    - 支持索引和查询优化
**
** 3. 使用方式：
**    ```sql
**    -- 函数形式：regexp(pattern, string)
**    SELECT regexp('^[a-z]+', 'hello');
**
**    -- 操作符形式：string regexp pattern
...
**
** 5. 性能特性：
**    - 使用非确定性有限自动机 (NFA) 进行匹配
**    - 性能上界为 O(N*M)，N 为正则表达式大小，M 为输入字符串大小
**    - 不会出现指数级性能行为（防止 ReDoS 攻击）
**    - 通过限制 NFA 大小防止拒绝服务攻击
**
** 6. 技术实现：
**    - 基于 Thompson NFA 构造算法
**    - UTF8 字符集的直接支持
**    - 内存使用优化和缓存友好设计
**    - 特殊优化版本的操作码（如 .* 的优化）
**
...
**
** 设计原则：
** - 安全性：防止 ReDoS（正则表达式拒绝服务）攻击
** - 正确性：完全符合 POSIX 扩展正则表达式标准
** - 效率性：在实际使用场景中的良好性能表现
** - 兼容性：与 SQLite 生态系统的无缝集成
** - 可维护性：清晰的代码结构和文档化实现
*/
**
