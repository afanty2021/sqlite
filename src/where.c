...
**
** SQLite WHERE 子句查询优化器实现
**
** 本模块包含用于处理 SQL 语句 WHERE 子句的 VDBE 代码生成。
** 此模块负责生成遍历表以查找适用行的循环代码。
** 在适用时选择并使用索引来加速搜索。由于此模块负责选择索引，
** 您也可以将此模块视为"查询优化器"。
**
** where.c 在 SQLite 查询处理流水线中的位置：
**
** 1. SQL 解析流程：SQL 文本 → 解析器 → 语法树 → where.c → 优化后的 VDBE 代码
**
**    - 输入：解析器生成的语法树和表结构信息
**    - 处理：WHERE 子句分析、索引选择、查询计划生成
**    - 输出：优化的 VDBE 字节码程序
**
**
** 2. 核心功能模块：
...
**
** 5. 特殊优化技术：
**    - ORDER BY LIMIT 优化：提前终止排序
**    - MIN/MAX 优化：直接查找最小/最大值
**    - EXISTS/IN 优化：半连接和存在性查询优化
**    - 子查询展开和物化策略
**
**
** 6. 虚拟表优化：
