...
** another is writing.
**
** SQLite 页面缓存子系统（Pager）实现
**
** 本文件是页面缓存子系统或称为 "pager" 的实现。
**
** Pager 用于访问数据库磁盘文件。它通过使用与数据库文件分离的日志文件
** 来实现原子性提交和回滚。Pager 还实现文件锁定，防止两个进程同时写入同一数据库文件，
** 或一个进程在另一个进程写入时读取数据库。
**
** Pager 在 SQLite 架构中的核心地位：
**
** 1. 页面缓存管理：
**    - 管理数据库页面的缓存池，提供高效的页面访问
**    - 实现页面的引用计数、脏页标记和生命周期管理
**    - 支持页面大小可配置和缓存容量管理
**    - 提供 LRU 替换策略和预取机制
**
** 2. 事务管理核心：
**    - 实现数据库事务的原子性（Atomicity）和一致性（Consistency）
**    - 管理事务的开始、提交、回滚和保存点操作
**    - 协调嵌套事务的并发控制和隔离级别
**    - 处理事务冲突和死锁检测
**
** 3. 日志子系统：
**    - 管理回滚日志（Rollback Journal）的创建、写入和清理
**    - 支持 WAL（Write-Ahead Logging）模式的实现
**    - 处理日志文件的同步和检查点操作
**    - 实现日志压缩和恢复机制
**
** 4. 并发控制：
...
**
** 2. 写前日志模式（WAL）：
**    - 将修改写入 WAL 文件，保持主文件不变
**    - 支持并发读写，提高多用户性能
**    - 通过检查点机制将 WAL 更新同步到主文件
**    - 适合高并发读写的场景
**
** 3. 内存模式（MEMORY）：
**    - 数据库完全存储在内存中
