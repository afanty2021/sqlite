...
**
** SQLite B-Tree 存储引擎实现
**
** 本文件使用 B-Tree 实现外部（基于磁盘的）数据库。
** 请参阅 "btreeInt.h" 的头部注释以获取更多信息，包括文件格式描述和操作概览。
**
** btree.c 在 SQLite 架构中的核心地位：
**
** 1. 存储引擎核心：
**    - 实现基于 B-Tree 的数据库文件存储格式
**    - 管理数据页的分配、读取、写入和缓存
**    - 支持表和索引的高效存储和检索
**    - 处理页面分裂、合并、平衡等维护操作
**
** 2. 文件系统抽象：
**    - 通过 VFS（虚拟文件系统）接口屏蔽底层文件系统差异
**    - 提供跨平台的文件操作抽象
**    - 处理文件锁定、并发控制和事务隔离
**    - 支持内存映射 I/O 和传统 I/O 两种模式
**
** 3. 事务管理：
**    - 实现数据库事务的 ACID 特性
**    - 管理 WAL（Write-Ahead Logging）和回滚日志模式
**    - 处理嵌套事务、保存点和回滚操作
**    - 协调与 Pager 模块的事务交互
**
** 4. 页面管理：
